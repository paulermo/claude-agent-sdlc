...
summarize() { # $1 = state file with a flat map of entries having .status
...
- docs/state/*.json is written ONLY by the PM orchestrator session (single-writer). If you are a dispatched agent: never edit state files — end with your report envelope instead.
